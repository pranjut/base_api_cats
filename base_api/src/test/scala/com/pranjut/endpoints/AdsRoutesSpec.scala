...
class AdsRoutesSpec extends BaseEndpointsSpec {
  ...
  val adService = mock[AdService[IO]]
  ...
  "AdsRoutes" should {
    "successfully insert ad" in {
      ...
      when(adService.insert(ad)).thenReturn(IO.pure(ad.copy(id = 1)))
    ...
    "successfully get ad" in {
      ...
      when(adService.get(1)).thenReturn(IO.pure(Some(ad)))
    ...
    "Delete ad" in {
      ...
      when(adService.delete(1)).thenReturn(IO.pure(1))
